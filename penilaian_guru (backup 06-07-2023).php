  ...
  ?>
    <div class="card-body">
      ...
      <h4 class="text-dark font-weight-500 mb-4">Data berikut merupakan Update Penilaian Tanggal
        <span class="text-danger"><?= date('d-m-Y', strtotime($log)) ?></span>, Jam <span class="text-danger"><?= date('H:i', strtotime($log)) ?></span>.<br>Untuk
              ...
              foreach ($queryGuru as $key => $data) : ?>
                <tr>
                  ...
                  ?>
                      ...
                      <td>
                        <?php if ($kriteria['kode_kriteria'] == 'C1') :
                          $value = '1';
                          if ($queryNilaiGuru->num_rows > 0) {
                            foreach ($queryNilaiGuru as $row) {
                              if ($row['id_guru'] == $data['id_guru'] && $row['id_kriteria'] == $kriteria['id_kriteria']) {
                                $value = $row['nilai_guru'];
                              }
                            }
                          }
                        ?>
                          <input type="number" name="kriteria-<?= $key ?>[]" class="form-control form-control-sm" style="width: 100px;" value="<?= $value ?>" min="0">
                        <?php
                        else : ?>
                          <select name="kriteria-<?= $key ?>[]" class="form-control form-control-sm" <?= $kriteria['kode_kriteria'] != 'C4' ? 'readonly' : '' ?>>
                            <?php
                            if ($kriteria['kode_kriteria'] == 'C2') {
